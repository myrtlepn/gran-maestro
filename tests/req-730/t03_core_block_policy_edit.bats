...
@test "AC-003 blocks mst policy edit from LLM Bash and records core_block" {
  sid="73003003-0000-4000-8000-000000000001"

  run run_pre_tool_hook "$sid" "mst policy edit default"

  [ "$status" -eq 2 ]
  [[ "$output" == *"[core-block]"* ]]
  [[ "$output" == *"mst policy edit"* ]]
  assert_core_block_event "$sid"
}

@test "AC-003 blocks mst policy install from LLM Bash and records core_block" {
  sid="73003003-0000-4000-8000-000000000002"

  run run_pre_tool_hook "$sid" "mst policy install ./policy.json"

  [ "$status" -eq 2 ]
  [[ "$output" == *"[core-block]"* ]]
  [[ "$output" == *"mst policy install"* ]]
  assert_core_block_event "$sid"
}
