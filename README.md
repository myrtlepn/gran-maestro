...
모든 MST provider 호출은 `models.providers.<provider>.premium_reasoning_effort`, `economy_reasoning_effort`, `default_reasoning_effort`와 호출별 `reasoning_effort`를 지원합니다. 호출별 값은 `default | inherit | low | medium | high | xhigh | max | ultra` 중 provider/model이 실제 지원하는 값만 사용할 수 있으며, 지원하지 않는 조합은 provider 시작 전에 차단됩니다. `default`는 선택된 tier의 추론 기본값을 먼저 사용하고, tier별 값이 없으면 provider fallback을 따릅니다. `inherit`은 native host 또는 CLI의 기본 추론 난이도를 그대로 사용합니다. 이 binding은 native 실행, direct external, Orca external에서 동일합니다.
