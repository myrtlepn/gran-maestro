...
- Sprint 2.2.3 자동 dispatch는 `config.agile.dispatch.enabled`와 `config.agile.dispatch.provider`를 기준으로 provider-neutral하게 분기합니다. `agile.dispatch.provider=claude`일 때만 이 `/mst:claude` managed path가 사용됩니다.
