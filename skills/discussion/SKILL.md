---
name: discussion
description: "사용자가 $mst:discussion 또는 /mst:discussion을 명시적으로 호출하거나 MST/Gran Maestro/Maestro의 discussion 기능 사용을 명시적으로 요청한 경우에만 실행합니다. 일반 요청에는 자동 활성화하지 않습니다."
user-invocable: true
...
<!-- mst-session-class: identity-required; root-source: existing DSC argument or new dsc allocation -->
...
2. canonical root가 `DSC-NNN`이면 bootstrap 결과의 `CANONICAL_ROOT_MST_ID`를 artifact ID로 그대로 사용합니다. 상속된 parent root가 다른 namespace일 때만 `MST_BOUND_SUBPROCESS`의 `python3 {PLUGIN_ROOT}/scripts/mst.py counter next --type dsc`로 child DSC ID를 한 번 예약합니다.

3. `{PROJECT_ROOT}/.gran-maestro/discussion/`와 `DSC-NNN/` 디렉토리를 생성합니다 (NNN은 3자리 zero-padded).
...
2. **participant + critic provider 동시 발송** (단일 응답):
   ...
   - `provider: "agy"`:
     ...
     Bash(
       run_in_background: true,
       command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" agy --print \"$(cat {absolute_path}/rounds/00/prompts/{participant.key}-prompt.md)\" --dangerously-skip-permissions > {absolute_path}/rounds/00/{participant.key}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/00/{participant.key}.md; exit $EC"
     )
     ```
   ...
   - `provider: "codex"`, external lane only:
     ...
     Bash(
       run_in_background: true,
       command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" codex exec --approve-for-me -m \"{model}\" -C $(pwd) \"$(cat {absolute_path}/rounds/00/prompts/critique-{criticKey}-prompt.md)\" > {absolute_path}/rounds/00/critique-{criticKey}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/00/critique-{criticKey}.md; exit $EC"
     )
     ```
   - `provider: "agy"`:
     ```
     Bash(
       run_in_background: true,
       command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" agy --print \"$(cat {absolute_path}/rounds/00/prompts/critique-{criticKey}-prompt.md)\" --dangerously-skip-permissions > {absolute_path}/rounds/00/critique-{criticKey}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/00/critique-{criticKey}.md; exit $EC"
     )
     ```
   - `provider: "claude"`, same-host native candidate:
     ```
...
- `provider: "codex"`, external lane only:
  ...
  Bash(
    run_in_background: true,
    command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" codex exec --approve-for-me -m \"{model}\" -C $(pwd) \"$(cat {absolute_path}/rounds/NN/prompts/{participant.key}-prompt.md)\" > {absolute_path}/rounds/NN/{participant.key}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/NN/{participant.key}.md; exit $EC"
  )
  ```
- `provider: "agy"`:
  ```
  Bash(
    run_in_background: true,
    command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" agy --print \"$(cat {absolute_path}/rounds/NN/prompts/{participant.key}-prompt.md)\" --dangerously-skip-permissions > {absolute_path}/rounds/NN/{participant.key}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/NN/{participant.key}.md; exit $EC"
  )
  ```
- `provider: "claude"`, same-host native candidate:
  ```
...
- `provider: "codex"`, external lane only:
  ...
  Bash(
    run_in_background: true,
    command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" codex exec --approve-for-me -m \"{model}\" -C $(pwd) \"$(cat {absolute_path}/rounds/NN/prompts/critique-{criticKey}-prompt.md)\" > {absolute_path}/rounds/NN/critique-{criticKey}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/NN/critique-{criticKey}.md; exit $EC"
  )
  ```
- `provider: "agy"`:
  ```
  Bash(
    run_in_background: true,
    command: "MST_SESSION_ID=\"{CANONICAL_MST_SESSION_ID}\" MST_CONTEXT_JSON=\"$(MST_CONTEXT_B64=\"{CANONICAL_MST_CONTEXT_B64}\" python3 -c 'import base64,os,sys;s=os.environ[\"MST_CONTEXT_B64\"];MAX_CONTEXT_BYTES=262144;sys.exit(\"encoded MST context exceeds limit\") if len(s)>349528 else None;raw=base64.b64decode(s.encode(\"ascii\"),altchars=b\"-_\",validate=True);sys.exit(\"decoded MST context is oversized or non-canonical\") if len(raw)>MAX_CONTEXT_BYTES or base64.urlsafe_b64encode(raw).decode(\"ascii\")!=s else None;sys.stdout.buffer.write(raw)')\" agy --print \"$(cat {absolute_path}/rounds/NN/prompts/critique-{criticKey}-prompt.md)\" --dangerously-skip-permissions > {absolute_path}/rounds/NN/critique-{criticKey}.md < /dev/null 2>&1; EC=$?; echo \"EXIT_CODE:$EC\" >> {absolute_path}/rounds/NN/critique-{criticKey}.md; exit $EC"
  )
  ```
- `provider: "claude"`, same-host native candidate:
  ```
