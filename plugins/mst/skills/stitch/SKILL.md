---
name: stitch
description: "사용자가 $mst:stitch 또는 /mst:stitch을 명시적으로 호출하거나 MST/Gran Maestro/Maestro의 stitch 기능 사용을 명시적으로 요청한 경우에만 실행합니다. 일반 요청에는 자동 활성화하지 않습니다."
user-invocable: true
...
<!-- mst-session-class: identity-required; root-source: existing DES argument or new des allocation -->
...
canonical root가 `DES-NNN`이면 bootstrap 결과의 `CANONICAL_ROOT_MST_ID`를 그대로 사용합니다. 상속된 parent root가 다른 namespace일 때만 `MST_BOUND_SUBPROCESS`의 `python3 {PLUGIN_ROOT}/scripts/mst.py counter next --type des`로 child DES ID를 한 번 예약합니다.
