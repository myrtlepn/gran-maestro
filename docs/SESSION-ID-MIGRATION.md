...
첫 번째 한계는 `claude --resume {uuid}` 시나리오다. 사용자가 동일 Claude Code 대화 UUID를 resume하더라도 새 프로세스 트리로 부트되면 PPID가 바뀐다. 기존 snapshot이나 request/plan JSON에 저장된 `owner_ppid`는 새 PPID와 불일치하므로 같은 대화의 자연 복구가 외부 세션처럼 보이거나, legacy fallback이 없으면 재식별이 실패할 수 있다. DOD-020의 목표인 "`claude --resume {uuid}` 시 자연 복구"는 PPID가 아니라 Claude Code `session_id`를 기준으로 해야 충족된다.
...
세 번째 한계는 `transcript_path` 기반 session_id fallback의 계약이 명확하지 않다는 점이다. Claude Code hook stdin JSON에 `session_id`가 들어오지 않는 환경에서도 transcript 파일명이 UUID인 경우가 있으나, fallback 우선순위와 실패 이벤트가 명세되지 않으면 세션 재개 중 snapshot을 찾지 못하고 silent drop으로 이어질 수 있다. DOD-020은 실패 시 `session_id_invalid` 또는 `session_id_mismatch_suspected`처럼 관찰 가능한 진단 이벤트를 요구한다.
...
| `hooks/mst-stop-hook.sh:1127` | Legacy fallback 주석 | request JSON에 `owner_session_id`가 없을 때 `owner_ppid`만 있는 파일을 warning과 함께 허용한다. |
| `hooks/mst-stop-hook.sh:1137` | `owner_ppid_value = "$PPID"` 비교 | non-terminal request가 현재 shell PPID 소유인지 판단하는 legacy fallback 비교다. |
| `hooks/mst-stop-hook.sh:1188` | Legacy fallback 주석 | plan JSON에도 request와 동일하게 `owner_ppid` only 파일 허용 경로가 남아 있다. |
| `hooks/mst-stop-hook.sh:1198` | `owner_ppid_value = "$PPID"` 비교 | non-terminal plan이 현재 shell PPID 소유인지 판단하는 legacy fallback 비교다. |
...
참고로 `hooks/mst-stop-hook.sh:401`의 `resolve_timeout_session_id()`, `scripts/_snapshot_probe.py:40`의 `resolve_session_id()`, `scripts/_snapshot_probe.py:30`의 `transcript_path` basename 추출, `scripts/mst_cmds/state.py:98`의 `_resolve_owner_session_id()`처럼 session_id 공존 경로가 이미 일부 존재한다. 그러나 위 표의 PPID fallback과 PID liveness가 남아 있는 동안에는 DOD-020의 "PPID는 디버그·감사 로그 보조 용도" 목표가 완전히 달성되지 않는다.

## 3. session_id 확보 경로

1차 경로는 Claude Code hook stdin JSON의 `session_id` 필드다. hook은 stdin payload를 JSON으로 파싱하고, 값이 비어 있지 않은 문자열이면 이를 canonical session identity 후보로 사용한다. 예시는 `{"session_id":"123e4567-e89b-42d3-a456-426614174000","transcript_path":"/path/to/123e4567-e89b-42d3-a456-426614174000.jsonl"}` 형태다. 이 값은 `MST_SESSION_ID` 또는 snapshot 생성 계층의 session key로 전달되어 `.gran-maestro/state/{uuid}/snapshot.json`을 찾는 데 사용된다.
...
Phase 1은 공존 단계다. snapshot과 durable request/plan JSON에 `owner_ppid`와 `owner_session_id`를 모두 기록한다. 검증은 기존 PPID 우선을 유지하고, `owner_session_id`는 로그와 진단에만 보조적으로 사용한다. 이 단계의 목적은 동작 변경 없이 실제 hook stdin의 `session_id` 확보율과 transcript fallback 성공률을 관찰하는 것이다.

Phase 2는 검증 단계다. `mst-pre-tool-use.sh`와 `mst-stop-hook.sh`에서 현재 hook stdin의 session_id, snapshot의 session id, durable resource의 `owner_session_id` 일치 여부를 비교하되 동작은 바꾸지 않는다. mismatch가 있으면 stderr와 `flow-detail.ndjson`에 warning을 기록한다. 이 단계에서 PPID와 session_id 판정 결과가 갈리는 실제 케이스를 수집한다.

Phase 3은 전환 단계다. owner 검증의 우선순위를 session_id로 바꾸고, PPID는 최종 fallback으로만 남긴다. UUID v4 검증 실패는 `session_id_invalid`, snapshot의 `created_at`과 transcript 시작 시각 delta가 비정상인 경우는 `session_id_mismatch_suspected` 이벤트로 기록한다. `claude --resume {uuid}` 경로는 같은 UUID의 `state/{uuid}/snapshot.json`을 자연스럽게 재사용해야 한다.

Phase 4는 마이그레이션 CLI 단계다. `mst.py state migrate` 또는 동등한 CLI가 기존 snapshot에 session_id가 없을 때 transcript_path 추적, bridge file, 사용자 안내를 통해 `owner_session_id`를 채운다. 실행 중에는 `.gran-maestro/state/.MIGRATING` sentinel을 만들고, 중단 후에는 `--resume`으로 이어가거나 `--rollback`으로 Phase N에서 N-1 상태로 되돌릴 수 있어야 한다.
...
session_id는 UUID v4 canonical lowercase string이어야 한다. 마이그레이션 최종 정책의 정규식은 다음 원문을 사용한다.

```text
^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$
```

검증 순서는 hook stdin의 `session_id`를 먼저 검사하고, 실패하면 `session_id_invalid` 이벤트를 기록한 뒤 `transcript_path` basename fallback을 시도한다. basename도 UUID v4 정책을 통과하지 못하면 공존 기간에는 PPID fallback으로 fail-open하되, fallback 사용 사실을 stderr와 `flow-detail.ndjson`에 남긴다. Phase 5 이후에는 PPID fallback을 owner 판정에 사용하지 않고 사용자가 hook stdin 또는 transcript 계약을 복구하도록 안내한다.
...
Sprint N+1은 Phase 1 공존 구현이다. snapshot과 request/plan JSON에 `owner_session_id` 필드를 추가하되 검증은 PPID 기준을 유지한다. 사용자 관찰 변화는 `jq '.owner_session_id' .gran-maestro/requests/*/request.json` 실행 시 UUID 또는 `null` 필드가 존재하는 것이다.
...
Sprint N+3은 Phase 3 session_id 우선 검증 구현이다. owner 판정은 `owner_session_id == current_session_id`를 먼저 보고, 실패 시에만 legacy PPID fallback을 탄다. 사용자 관찰 변화는 `claude --resume {uuid}` 이후 같은 `state/{uuid}/snapshot.json`이 재사용되고, UUID 형식 실패 시 `session_id_invalid` 이벤트가 보이는 것이다.
