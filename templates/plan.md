...
{최초 요청에서 사용자가 이루고자 한 결과와 이유. 구현 방법이나 후속 리뷰 제안은 포함하지 않음}

## 요청 (Refined)

{사용자 대화로 명확해진 의도. 최초 의도를 유지하면서 목표·범위·우선순위·제약을 정제한 내용}

> `사용자 최초 의도`와 `요청 (Refined)`까지가 하위 단계에 전달되는 canonical Intent Anchor입니다.
> 아래의 실행 계획, 기술 선택, PAC, 리뷰 제안은 Intent Anchor를 실현·검증하기 위한 수단이며 사용자 의도로 승격하지 않습니다.
