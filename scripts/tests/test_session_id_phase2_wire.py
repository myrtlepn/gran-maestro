...
def hook_payload(session_id: str = SID_A) -> dict:
    return {
...
def test_pre_tool_use_mismatch_warning(tmp_path: Path) -> None:
    ...
    result = run_hook(PRE_TOOL_HOOK, tmp_path, hook_payload())
    events = mismatch_events(tmp_path)
...
def test_pre_tool_use_mismatch_warning_dedup_by_ppid_and_session(tmp_path: Path) -> None:
    ...
    first = run_hook(PRE_TOOL_HOOK, tmp_path, hook_payload())
    second = run_hook(PRE_TOOL_HOOK, tmp_path, hook_payload())
